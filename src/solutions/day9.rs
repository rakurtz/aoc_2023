...
pub fn run() {
    ...
    let result_pt1 = all_lines.sum_of_next_values();
...
impl Line {
    ...
    fn calculate_next_value(&mut self) {
        ...
        self.next = Some(rows.iter().fold(0, |akk, x| akk + x.last().unwrap()));
    }
...
impl AllLines {
    fn new_from_input(input: &str) -> Self {
        ...
        let mut lines = AllLines {
            lines,
        };

        lines.calculate_all_next_values();

        lines
    }

    fn calculate_all_next_values(&mut self) {
        self.lines.iter_mut().for_each(|line| line.calculate_next_value());
    }

    fn sum_of_next_values(&self) -> isize {
        self.lines.iter().fold(0, |akk, x| akk + x.next.unwrap())
    }
...
10 13 16 21 30 45";
        ...
        // assert_eq!(28, 0);
        // assert_eq!(68, 0);
        assert_eq!(114, all_lines.sum_of_next_values());
